...
const useStarredRepoStore = create<StarredRepoStoreState, []>(
    (persist as PersistStore)((set, get): StarredRepoStoreState => ({
        repos: [],
        toggleStarredStatusForRepo: (id: number, starred: boolean) => set(()=>{
			const reposFromStore = get().repos;
            return ({
                repos: starred ? reposFromStore.filter(repo=>repo !== id) : [...reposFromStore, id]
            })}
        ),
    }),
    {
